...
const BannerModal = ({ isOpen, onClose, item, onSubmit, isLoading }) => {
  ...
  const handleFile = (e) => {
    ...
    // Clean up previous unsaved upload before uploading new one
  ...
  // ── Delete: remove from Supabase first, then clear state ─────────────────────
  ...
  const handleConfirmClose = () => {
    // Cleanup unsaved upload from Supabase
    if (imageUrl && imageUrl !== originalImageUrl.current) {
      deleteImg(imageUrl);
    }
    setShowWarning(false);
  ...
  return (
    <>
      <MasterDataModal
        isOpen={isOpen}
        onClose={handleClose}
        title={item ? 'Edit Banner' : 'Tambah Banner'}
        maxWidth="max-w-lg"
      >
      ...
      <UnsavedChangesModal
        ...
        description="Form banner ini memiliki perubahan yang belum disimpan. Jika ditutup, semua perubahan akan hilang."
      />
    </>
  );
};
